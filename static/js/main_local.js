...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Expose folder monitors globally so modules can check monitoring state
    window.swvFolderMonitor = swvFolderMonitor;
    window.cvFolderMonitor = cvFolderMonitor;
...
function setupNavigation(uiManager) {
    const swvBtn = getById('swvBtn');
    const cvBtn = getById('cvBtn');

    bindIfPresent(swvBtn, 'click', () => {
        uiManager.showScreen('swvAnalysisScreen');
    });

    bindIfPresent(cvBtn, 'click', () => {
        uiManager.showScreen('cvAnalysisScreen');
    });

    getAll('[id^="backToWelcome"]').forEach((button) => {
        bindIfPresent(button, 'click', () => {
            uiManager.showScreen('welcomeScreen');
        });
    });
}
