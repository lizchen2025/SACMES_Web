...
export class DiagnosticsPanel {
    ...
    setupUI() {
        ...
        if (navButtons) {
            ...
            diagButton.className = 'bg-gray-600 hover:bg-gray-700 text-white px-6 py-2 rounded transition duration-200';
    ...
    displayDiagnostics(data) {
        ...
        if (data.status === 'success') {
            ...
            html += `<div class="ml-4 text-xs">SWV Analyzer: ${diag.analyzers.swv_analyzer_loaded ? '✓ Loaded' : '✗ Not Loaded'}</div>`;
            html += `<div class="ml-4 text-xs">CV Analyzer: ${diag.analyzers.cv_analyzer_loaded ? '✓ Loaded' : '✗ Not Loaded'}</div>`;
    ...
    displayFileReadResults(data) {
        ...
        if (data.status === 'success') {
            ...
            html += `<div>Safety Check: ${result.safety_check ? '✓ Passed' : '✗ Failed'}</div>`;
