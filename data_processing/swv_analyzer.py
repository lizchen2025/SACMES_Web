...
def analyze_swv_data(file_path, analysis_params, selected_electrode=None):
    ...
    if selected_electrode is not None:
        # Inspect the file once without electrode filtering so we can distinguish
        # "requested electrode not present in this file" from a generic no-data case.
        unfiltered_result = ReadData(
            myfile=file_path,
        ...
        detected_electrodes = unfiltered_result.get('detected_electrodes', 0)

        if detected_electrodes <= selected_electrode:
            return {
                "status": "error",
                "error_type": "electrode_not_present_in_file",
                "message": (
                    f"Requested electrode {selected_electrode + 1} is not readable in this file. "
                    f"The file appears to contain {detected_electrodes} readable electrode(s)."
                ),
        ...
        # Check electrode data exists after filtering
        if selected_electrode in electrodes_data:
            ...
            detected_electrodes = electrodes_data.get('detected_electrodes', detected_electrodes)
            return {
                "status": "error",
                "error_type": "electrode_not_present_in_file",
                "message": (
        ...
        if not potentials or not currents:
            return {
                "status": "warning",
                "warning_type": "electrode_no_valid_data",
                "message": f"No valid data found for electrode {selected_electrode + 1} in this file.",
                "detected_electrodes": detected_electrodes,
                "requested_electrode": selected_electrode + 1,
                "potentials": [],
                "raw_currents": [],
    ...
    if not potentials or not currents or not any(abs(c) > 1e-15 for c in currents):
    ...
    # Get voltage range parameters with safe defaults (use full data range if not specified)
    if current_freq > cutoff_frequency:
        xstart_val = analysis_params.get('high_xstart')
        xstart_val = xstart_val if xstart_val is not None else max_potential
        xend_val = analysis_params.get('high_xend')
        xend_val = xend_val if xend_val is not None else min_potential
    else:
        xstart_val = analysis_params.get('low_xstart')
        xstart_val = xstart_val if xstart_val is not None else max_potential
        xend_val = analysis_params.get('low_xend')
        xend_val = xend_val if xend_val is not None else min_potential
    ...
    if analysis_params['SelectedOptions'] == "Peak Height Extraction":
        ...
        if adjusted_smoothed_currents and adjusted_potentials:
            ...
            # abs() on the final trapz result handles descending-potential scans
            # where dx < 0 would otherwise make the integral negative.
            peak_value = abs(np.trapz([abs(d) for d in diff_currents], adjusted_potentials))
